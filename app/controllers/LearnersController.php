 ...
 * @category  Controller
 */
class LearnersController extends SecureController {
    function __construct() {
        parent::__construct();
        $this->tablename = SqlTables::tbl_learners;
        $this->fields = [
            'id', 'name', 'dob', 'school_id', 'grade', 'cellphone', 'created', 'modified'
        ];
    }
    ...
    function index($fieldname = null, $fieldvalue = null) {
        ...
        $this->view->page_title = 'Learners';
        $this->view->render('learners/list.php', $data, 'main_layout.php');
    }
    ...
    function view($rec_id) {
        ...
        if (!empty($record)) {
            $this->view->page_title = 'View Learner: ' . $record['name'];
            $this->view->render('learners/view.php', $record, 'main_layout.php');
        } else {
            if ($db->getLastError()) {
                $this->view->page_error = $db->getLastError();
            } else {
    ...
    function manage($rec_id = null) {
        ...
        if (is_post_request()) {
            ...
            $rules_array['dob'] = 'required';
            $rules_array['school_id'] = 'required|numeric';
            $rules_array['grade'] = 'required|numeric';
            $rules_array['cellphone'] = 'required|numeric';
            ...
            ajaxFormPostOutcome($errors, get_link("learners"));
            return;
        }
        ...
        if (!empty($rec_id)) {
            ...
            if ($data) {
                ...
                $this->view->page_title = "Edit Learner: " . $data['name'];
            } else {
                if ($db->getLastError()) {
                    $this->view->page_error[] = $db->getLastError();
                } else {
            ...
            $data = null;
            $this->view->page_title = "Add Learner";
        }

        $this->view->render('learners/add.php', $data, 'main_layout.php');
    }
