...
function get_active_user_role() {
    ...
    $user = $model->get_user(array("id" => get_active_user("id")));
    return $user['role'];
}
...
function authenticate_user() {
    ...
    if (user_login_status() == true) {
        ...
        if (is_post_request()) {
            ...
            $model = new AccountModel;
            $user = $model->get_user(array("user_name" => $post_data['username']));

            if (!empty($user)) {
                if (password_verify($post_data['password'], $user['user_password_hash'])) {
                    set_session("user_data", $user);
                    redirect_to_page("index");
                    exit;
                } else {
        ...
        $view->render("index/login.php", null, "login_layout.php");
        exit;
    }
}
 ...
 * @return string
 */
function print_lang($name, $echo = true) {
    global $lang;
...
function get_currency() {
    ...
    return (!empty($core_settings) && !empty($core_settings['currency']) ? $core_settings['currency'] : '$');
}

function print_currency() {
    echo get_currency();
}

function get_num_decimal() {
    global $core_settings;
    return (!empty($core_settings) && !empty($core_settings['money_currency_position']) ? $core_settings['money_currency_position'] : 2);
}

function print_num_decimal() {
    echo get_num_decimal();
}
...
function number_display($value) {
    return number_format($value, get_num_decimal());
}

function ajaxFormPostOutcome($errors, $redirectUrl = null, $success_message = null) {
    if (!empty($errors)) {
        if (is_array($errors)) {
            $msg = "<ul>";
...
function isActiveController($controllers, $check_controller, $return_class = "show") {
    if (!is_array($controllers)) {
        $controllers = array($controllers);
    }

    return (in_array($check_controller, $controllers) ? $return_class : "");
}

function isActiveAction($controllers, $actions, $check_controller, $check_action, $return_class = "active") {
    if (!is_array($actions)) {
    ...
    if (!in_array($check_controller, $controllers)) {
        return '';
    }

    if (in_array($check_action, $actions)) {
        return $return_class;
    }
