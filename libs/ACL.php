...
class ACL {
     ...
     * @var array
     */
    public static $exclude_page_check = array(
        "", 
        "index/index", 
        "index/login", 
        "index/register",
    ...
    public static function GetPageAccess($path) {
        ...
        $action = (!empty($arr_path[1]) ? $arr_path[1] : "index");
        
        //If User is accessing exclude access check page
        if (in_array("$page/$action", self :: $exclude_page_check) || in_array("$page", self :: $exclude_page_check)) {
            return AUTHORIZED;
        }
